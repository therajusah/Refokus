       ...
       <img
        ...
        <div className="flex ml-20 links gap-14">
          {["Home", "Work", "Culture", "", "News"].map((elem, index) => (
            <a key={index} href={`#${elem}`} className="flex items-center gap-1 text-sm font-regular">
              {elem.length === 0 ? (
                <span className="w-[2px] h-7 bg-zinc-700"></span>
              ) : (
                <>
                  {index === 1 && (
                    <span style={{ boxShadow: "0 0 0.35em #00FF19" }} className="inline-block w-2 h-2 bg-green-500 rounded"></span>
                  )}
                  {elem}
                </>
              )}
